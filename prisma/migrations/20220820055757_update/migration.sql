/*
  ...
  - You are about to drop the column `creatorProfileUrl` on the `Game` table. All the data in the column will be lost.
  - Added the required column `authorProfileUrl` to the `Game` table without a default value. This is not possible if the table is not empty.
  - Made the column `author` on table `Game` required. This step will fail if there are existing NULL values in that column.
